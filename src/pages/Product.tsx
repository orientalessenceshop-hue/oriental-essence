...
const Product = () => {
  ...
  // === Fetch reviews + calculate rating
  const fetchStats = async () => {
    ...
    if (!error && data) {
      ...
      const avg = realCount > 0 ? Number((realSum / realCount).toFixed(1)) : 0;
      setProductRating({ count: realCount, avg });
    } else {
      setProductRating({ count: 0, avg: 0 });
    }
  };
  ...
  useEffect(() => {
    ...
    const listener = (e: CustomEvent) => {
      if (e.detail.productId === id) fetchStats();
    };
    window.addEventListener("reviewAdded", listener as any);
    return () => window.removeEventListener("reviewAdded", listener as any);
  }, [id]);
      ...
      <section className="py-12 flex-1">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Product image */}
            <div className="flex items-center justify-center bg-white rounded-2xl shadow-lg p-6 hover:shadow-2xl transition-shadow">
              <img
